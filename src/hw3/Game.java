...
public class Game
{
	...
	/**
	 * Random seed instance variable
	 */
	private Random rand;
	
	/**
	 * The value of the new tile generated
	 */
	private int newTile;
	
	/**
	 * Makes a GameUtil object that allows it to be used 
	 */
	private GameUtil newGame;
	
	/**
	 * Makes a two dimensional array of all the values
	 */
	private int[][] grid;
	...
	/**
	 * Keeps track of whether the grid can be undone
	 */
  ...
  {
	newGame = givenConfig;
	grid = newGame.initializeNewGrid(givenSize, givenRandom);
	previousGrid = newGame.initializeNewGrid(givenSize, givenRandom);
    gridSize = givenSize;
    rand = givenRandom;
    isUndo = false;
    lastMove = null;
    newTile = newGame.generateRandomTileValue(rand);
  ...
  {
 	  previousGrid = newGame.copyGrid(grid);
 	  ArrayList<Move> moves = new ArrayList<Move>();
 	  ArrayList<Move> change = new ArrayList<Move>();
 	  int i = 0;
 	  int j = 0;
 	  while(i < gridSize)//Loop through the entire grid
 		  {
 			  int[] temp = copyRowOrColumn(i,dir);
 			  change.addAll(newGame.shiftArray(temp));
 			  for(j = 0; j <change.size(); ++j) { //Loops through each column or row in the grid
 				  change.get(j).setDirection(i, dir);
 			  }
  ...
  {
	TilePosition nextTile = new TilePosition(0,0,0);
    if(lastMove == null) {
    	isUndo = true;
    	return null;
    }
    else {
    	nextTile = newGame.generateRandomTilePosition(grid, rand, lastMove);
    	nextTile.setValue(newTile);
    	newTile = newGame.generateRandomTileValue(rand);
    	setCell(nextTile.getRow(), nextTile.getCol(), nextTile.getValue());
    	score = newGame.calculateTotalScore(grid);
    	isUndo = false;
    }
